...
class TelloNode():
    def __init__(self, node):
        ...
        self.node.get_logger().info('Tello: Battery: ' + str(self.tello.get_battery()) + '%')
    ...
    def setup_subscribers(self):
        ...
        self.sub_move = self.node.create_subscription(
            String,           # message type
            'move',           # topic name
            self.cb_move,     # your new callback
            1
        )
    ...
    def cb_move(self, msg: String):
        data = msg.data.split()
        direction, dist_s = data
        direction, dist_s = data
        try:
            dist = int(dist_s)
        except ValueError:
            self.node.get_logger().warn(f"bad distance: '{dist_s}'")
            return

        # map direction to Tello API
        if direction == 'forward':
            self.node.get_logger().info(f"Moving forward {dist}cm")
            self.tello.move_forward(dist)
        elif direction == 'back':
            self.node.get_logger().info(f"Moving back {dist}cm")
            self.tello.move_back(dist)
        elif direction == 'left':
            self.node.get_logger().info(f"Moving left {dist}cm")
            self.tello.move_left(dist)
        elif direction == 'right':
            self.node.get_logger().info(f"Moving right {dist}cm")
            self.tello.move_right(dist)
        elif direction == 'up':
            self.node.get_logger().info(f"Moving up {dist}cm")
            self.tello.move_up(dist)
        elif direction == 'down':
            self.node.get_logger().info(f"Moving down {dist}cm")
            self.tello.move_down(dist)
        else:
            self.node.get_logger().warn(f"Unknown move direction: '{direction}'")
